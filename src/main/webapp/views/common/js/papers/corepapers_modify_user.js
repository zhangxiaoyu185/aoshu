...
function getInfo(id){
	...
	getOData(str,"corePapers/views",{fn:function(oData){
		if(oData.code == 1) {
			userUuids = oData.data.crpesUser;
			if(!userUuids) {
				userUuids = '';
			}
		} else {
...
function Modify(){
	...
	var crpesUser = userUuids;
	var str = 'crpesUuid='+encodeURIComponent(crpesUuid)+'&crpesUser='+encodeURIComponent(crpesUser);
	getOData(str,"corePapers/update/corePapers/user",{
		fn:function(oData){
		},
