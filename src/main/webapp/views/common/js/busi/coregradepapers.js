...
$(function () {
	...
	$('.addit').on('click', function(){
		layer.open({
			...
			content: '../busi/coregradepapers_add.html?grade='+getQueryString("gradeId")
		});
	});
	//修改
	$("body").delegate('.modifyit','click', function(){
		layer.open({
			...
			shadeClose: false, //点击遮罩关闭层
			area : [widthLayer , heightLayer],
			content: '../busi/coregradepapers_modify.html?id='+$(this).attr("data-id")
		});
	});
	//全选
	...
	$("body").delegate(".delthese","click",function(){
		...
		if(ids == ""){
			...
			if (r==true){
				var str = 'crgpsUuids='+encodeURIComponent(ids);
				getOData(str,"coreGradePapers/delete/batch",{fn:function(oData){
						var obj = {};//查询条件对象
						searchContent(obj);
	...
	$("body").delegate(".delit","click",function(){
		...
		if (r==true){
			var str = 'crgpsUuid='+encodeURIComponent($(this).attr("data-id"));
			getOData(str,"coreGradePapers/delete/one",{fn:function(oData){
					var obj = {};//查询条件对象
					searchContent(obj);
					pagenum = parseInt($(".curpage").text());
					isNull(obj,pagenum);
...
function showList(obj,pagenum){
	var aData =[{name:"<input type='checkbox' name='checkboxAll' value='checkbox' />",percent:"5"},
				...
				{name:"操作",percent:"20"}];
	setTableHead(aData);
	var str = 'pageNum='+pagenum+'&pageSize=10&crgpsGrade='+getQueryString("gradeId");
	if(obj.crpesName!=""){
	...
	getOData(str,"coreGradePapers/find/by/cnd",{fn:function(oData){
		var strhtml_list = "";
		var arrData = oData.data.result;
		var ln = arrData.length;
		for(var i=0;i<ln;i++){
			strhtml_list += '<tr class="trHighLight">'
				+'<td>'+'<input type="checkbox" name="checkbox" value="checkbox" data-id="'+arrData[i].crgpsUuid+'"/>'+'</td>'
				+'<td>'+(arrData[i].crpesName || "")+'</td>'
				+'<td>'+(arrData[i].crpesScore || "")+'</td>'
				+'<td>'+(arrData[i].crpesClassName || "")+'</td>'
				+'<td>'+(arrData[i].crgpsOrd)+'</td>'
				+'<td>'+(arrData[i].crgpsGqsj)+'</td>'
				+'<td>'
				+'<a  class="p-edit modifyit" data-id="'+arrData[i].crgpsUuid+'">修改</a>'
				+'<a  class="p-edit delit" data-id="'+arrData[i].crgpsUuid+'">删除</a>'
				+'</td>'
				+'</tr>';
		}
