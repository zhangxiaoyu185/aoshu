...
$(function () {
	...
	$("body").delegate('.detailit','click', function(){
		layer.open({
			...
			content: '../courseerror/corecourse_detail.html?id='+$(this).attr("data-id")
		});
	});
	//新增
	$('.addit').on('click', function(){
		layer.open({
			...
			shadeClose: false, //点击遮罩关闭层
			area : [widthLayer , heightLayer],
			content: '../courseerror/corecourse_add.html'
		});
	});
	//修改
	$("body").delegate('.modifyit','click', function(){
		layer.open({
			...
			shadeClose: false, //点击遮罩关闭层
			area : [widthLayer , heightLayer],
			content: '../courseerror/corecourse_modify.html?id='+$(this).attr("data-id")
		});
	});
	//全选
	...
	$("body").delegate(".delthese","click",function(){
		...
		if(ids == ""){
			...
			if (r==true){
				var str = 'crcreUuids='+encodeURIComponent(ids);
				getOData(str,"coreCourse/delete/batch",{fn:function(oData){
						var obj = {};//查询条件对象
						searchContent(obj);
	...
	$("body").delegate(".delit","click",function(){
		...
		if (r==true){
			var str = 'crcreUuid='+encodeURIComponent($(this).attr("data-id"));
			getOData(str,"coreCourse/delete/one",{fn:function(oData){
					var obj = {};//查询条件对象
					searchContent(obj);
					pagenum = parseInt($(".curpage").text());
					isNull(obj,pagenum);
...
function showList(obj,pagenum){
	...
	if(obj.crcreName==""){
		...
		str = 'pageNum='+pagenum+'&pageSize=20&crcreName='+encodeURIComponent(obj.crcreName);
	}
	if(obj.crcreClass!='0'){
		str += '&crcreClass='+encodeURIComponent(obj.crcreClass);
	}
	getOData(str,"coreCourse/find/by/cnd",{fn:function(oData){
		var strhtml_list = "";
		var arrData = oData.data.result;
		var ln = arrData.length;
		for(var i=0;i<ln;i++){
			strhtml_list += '<tr class="trHighLight">'
				+'<td>'+'<input type="checkbox" name="checkbox" value="checkbox" data-id="'+arrData[i].crcreUuid+'"/>'+'</td>'
				+'<td>'+(arrData[i].crcreName || "")+'</td>'
				+'<td>'+(arrData[i].crcreContent || "")+'</td>'
				+'<td>'+(arrData[i].crcreClassName || "")+'</td>'
				+'<td>'
				+'<a  class="p-edit detailit" data-id="'+arrData[i].crcreUuid+'">查看</a>'
				+'<a  class="p-edit modifyit" data-id="'+arrData[i].crcreUuid+'">修改</a>'
				+'<a  class="p-edit delit" data-id="'+arrData[i].crcreUuid+'">删除</a>'
				+'</td>'
				+'</tr>';
		}
