...
$(function () {
		...
		$("body").delegate('.detailit','click', function(){
			layer.open({
		        ...
		        content: '../question/knowledge_detail.html?id='+$(this).attr("data-id")
		    });
		});
		//新增
		$('.addit').on('click', function(){
			layer.open({
		        ...
		        shadeClose: false, //点击遮罩关闭层
		        area : [widthLayer , heightLayer],
		        content: '../question/knowledge_add.html'
		    });
		});
		//修改
		$("body").delegate('.modifyit','click', function(){
			layer.open({
		        ...
		        shadeClose: false, //点击遮罩关闭层
		        area : [widthLayer , heightLayer],
		        content: '../question/knowledge_modify.html?id='+$(this).attr("data-id")
		    });
		});
	...
	function showList(obj,pagenum){
		var aData = [{name:"<input type='checkbox' name='checkboxAll' value='checkbox' />",percent:"5"},
		             {name:"知识点名称",percent:"20"},
		             {name:"所属大知识点",percent:"20"},
		             {name:"所属类别",percent:"20"},
		             {name:"操作",percent:"35"}];
		setTableHead(aData);
		var str = '';
		if(obj.crkleName==""){
		...
		getOData(str,"coreKnowledge/find/by/cnd",{fn:function(oData){
					...
					for(var i=0;i<ln;i++){
						strhtml_list += '<tr class="trHighLight">'
							...
							+'<td>'+(arrData[i].crkleFatherName || "")+'</td>'
