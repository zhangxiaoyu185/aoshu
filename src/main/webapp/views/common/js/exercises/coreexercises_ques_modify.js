...
//				}
	...
	$("body").delegate(".searchBtn","click",function(){
		...
		ModifyData('code_exercises',  $(".crqtsCode").val());
		ModifyData('class_exercises',  $(".crqtsClass").val());
		pageNum_search = 1;
		ModifyData('num_exercises',  pageNum_search);
	});
	//重置
...
function listProblem(problems){
	...
	for ( var i in problems) {
		...
		strhtml_list += '<td>'+(problems[i].crcasName || "")+'</td>'
			+'<td><a  class="p-edit delit" data-id="'+problems[i].crqtsCode+'">取消</a>'
				+'<a  class="p-edit up" data-index="'+i+'" data-id="'+problems[i].crqtsCode+'">上移</a>'
				+'<a  class="p-edit next" data-index="'+i+'" data-id="'+problems[i].crqtsCode+'">下移</a></td>'
			+'</tr>';
	}
...
function Modify(msgObject){
	...
	crecsContent = JSON.stringify(content);
	var str = 'crecsUuid='+encodeURIComponent(crecsUuid)+'&crecsContent='+encodeURIComponent(crecsContent);
	getOData(str,"coreExercises/update/coreExercises/by/ques",{
		fn:function(oData){
...
function getDataCode(){
	var str = 'crcdaUuid=code_exercises';
...
function getDataClass(){
	var str = 'crcdaUuid=class_exercises';
	getOData(str,"coreCacheData/views",{fn:function(oData){
		if(oData.code == 1) {
...
//获取缓存详情
function getDataNum(){
	var str = 'crcdaUuid=num_exercises';
	getOData(str,"coreCacheData/views",{fn:function(oData){
		if(oData.code == 1) {
